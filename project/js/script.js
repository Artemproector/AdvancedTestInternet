const CONFIG = {
    express_test: ["https://ya.ru"],
    // Сайты для проверки доступности по категориям
    categories: {
        ru1: {
...
function scrollToActiveProgress() {
    ...
    if (activeElement) {
        ...
        // Прокручиваем так, чтобы элемент оказался по центру
...
function openSettings() {
    ...
    area.innerHTML = `<ul>
            <li>
                <div onclick="ShowSettings('general')">Общие</div>
            </li>
            <li>
                <div onclick="ShowSettings('tests')">Управление тестированием</div>
            </li>
            <li>
                <div onclick="ShowSettings('history')">Управление историей</div>
            </li>
            <li>
                <div onclick="ShowSettings('profi')">Только для профессионалов!</div>
            </li>
        </ul>`
}
...
<div class="help-section">
    ...
    <div class="mode-item">
        ...
        <p><strong>Все сайты в этой категории доступны и работаютв штатном режиме.</strong>  При открытии страницы проблем не возникает. Это означает, что ваш провайдер или сетевое оборудование не ограничивает доступ к данным ресурсам.</p>
    </div><div class="mode-item">
        <strong>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="display: inline-block; vertical-align: middle; margin-right: 6px;">
    ...
    <div class="mode-item">
        ...
        <p><strong>Некоторые сайты в категории недоступны, но часть работает.</strong>
            Это может указывать на то, что провайдер или администратор сети заблокировал только определённые ресурсы (черные списки).</p>
        <div class="tip">
            ...
            <div class="help-section">
                ...
                <div class="tip">
                    Если тест показал "Белые списки" — скорее всего, ваш провайдер блокируют зарубежные сайты.
                </div>
            </div>`
}
// Здесь раньше была функция openHistory()
// Но она была лишней, так что я её удалил ¯\_(ツ)_/¯
// historyload() вызывается напрямую из меню. Она кстати в самом низу
// На момент написания этого комента она на 1388 строке (инлайн)
function openlink() {
    //window.location.href = 'https://github.com/Artemproector/AdvancedTestInternet';
    fullnavbar.classList.add('show-nav');
    let fullLabel = document.querySelector('.labal-full-menu');
    if (fullLabel) {
...
async function checkupdate() {
    try {
        const response = await fetch('https://api.github.com/repos/Artemproector/AdvancedTestInternet/releases/latest', {
...
function openupdate() {
    ...
    area.innerHTML = `
        <div style="text-align: center; padding: 40px 20px;">
            <p style="color: #8892b0;">⏳ Проверка обновлений...</p>
        </div>
    `;
        ...
        if (result.status === 'up_to_date') {
            ...
            area.innerHTML = `
                <div style="text-align: center; padding: 40px 20px;">
                    ...
                    <p style="color: #8892b0;">
                        Последняя версия: ${result.latestVersion}
                    </p>
                    <br>
            ...
            area.innerHTML = `
                <div style="text-align: center; padding: 40px 20px;">
                    ...
                    <button onclick="openupdate()" style="
                        display: inline-block;
                        padding: 10px 25px;
                        background: #2a3555;
                        color: #fff;
...
// 4. ТЕСТ СКОРОСТИ
// ============================================================
async function testDownloadSpeed(url, attempts = 3) {
    const results = [];
    let totalBytes = 0;
    ...
    try {
        ...
        for (let i = 0; i < attempts; i++) {
            try {
                ...
                const response = await fetch(url, {
                    signal: AbortSignal.timeout(5000) // 5 секунд на попытку
                });
        ...
        if (successfulAttempts === 0) {
            return {
                success: false,
                speedBps: 0,
                error: 'Все попытки загрузки провалились',
                results: results
            };
        }

        // Вычисляем среднюю скорость (только по успешным попыткам)
        ...
        return {
            ...
            results: results,
            stability: stability,
            quality: stability > 0.8 ? 'Стабильно' :
                stability > 0.5 ? 'Нестабильно' : 'Очень нестабильно',
            stdDev: stdDev
        };
...
async function testUploadSpeed(url, attempts = 3) {
    const results = [];
    let successfulAttempts = 0;
    const testDataSize = 1 * 1024 * 1024; // 1 МБ
    ...
    try {
        for (let i = 0; i < attempts; i++) {
            try {
                ...
                // Отправляем как FormData (работает с httpbin)
...
// 5. ТЕСТ ПИНГА
// ============================================================
async function testPing(url = 'https://www.avito.st/dstatic/favicon.ico', attempts = 3) {
    const pings = [];

    for (let i = 0; i < attempts; i++) {
        const result = await checkSiteAvailability(url, 3000);
        if (result.success) {
    ...
    if (average <= 30) {
        updateDisplay("ping", 5)
    }
    if (average >= 31 && average <= 60) {
        updateDisplay("ping", 4)
    }
    if (average >= 61 && average <= 100) {
        updateDisplay("ping", 3)
    }
    if (average >= 101 && average <= 200) {
        updateDisplay("ping", 2)
    }
    if (average >= 200) {
...
// ============================================================
// 6. БЫСТРАЯ ПРОВЕРКА НАЛИЧИЯ ИНТЕРНЕТА
// ============================================================
async function quickInternetCheck() {
    const testUrls = CONFIG.express_test

    const results = await Promise.all(
        testUrls.map(url => checkSiteAvailability(url, 2000))
    );
...
// ============================================================
// 7. ОПРЕДЕЛЕНИЕ РЕЖИМА РАБОТЫ СЕТИ
// ============================================================
...
/**
 * Обновляет дисплей
 * @param {string} type - Тип дисплея: 'dwn', 'up', 'ping', 'mode', 'all'
 * @param {number} value - Значение от 0 до 5 (сколько элементов зажечь)
 */
function updateDisplay(type, value = 0) {
    // Приводим значение к числу и ограничиваем от 0 до 5
...
async function runFullTest() {
    ...
    progress_el_1.classList.remove('pr-bar-section--pass')
    progress_el_2.classList.remove('pr-bar-section--pass')
    progress_el_3.classList.remove('pr-bar-section--pass')
    progress_el_4.classList.remove('pr-bar-section--pass')
    progress_el_5.classList.remove('pr-bar-section--pass')
    progress_el_1.classList.remove('pr-bar-section--fail')
    progress_el_2.classList.remove('pr-bar-section--fail')
    progress_el_3.classList.remove('pr-bar-section--fail')
    progress_el_4.classList.remove('pr-bar-section--fail')
    progress_el_5.classList.remove('pr-bar-section--fail')
    ...
    progress_el_5.classList.remove('pr-bar-section--active')
    scrollToFirst()
    try {
        // ============================================================
    ...
    <circle cx="12" cy="17" r="1" fill="#ffa502"/>
        ...
        const downloadResult = await testDownloadSpeed(CONFIG.speedTest.download, 3);
        const uploadResult = await testUploadSpeed(CONFIG.speedTest.upload, 3);
        const pingResult = await testPing('https://www.avito.st/dstatic/favicon.ico', 5);
        ...
        // Обновляем UI скорости загрузки
        if (uploadResult.success) {
            ...
            document.getElementById('uploadSpeed').innerHTML = '-- <span class="unit">Мбит/с</span>'; 3
            updateDisplay('up', 1);
            progress_el_2.classList.add('pr-bar-section--fail')
        }
        ...
        if (pingResult.success) {
            ...
            document.getElementById('ping').innerHTML = `
                ...
                <span style="color: ${pingColor.color}; font-size: 12px; margin-left: 8px;">
                </span>
            `;
    ...
    <circle cx="12" cy="17" r="1" fill="#ffa502"/>
        ...
        if (downloadResult.success) {
            ...
            if (speedHistory.length > 50) {
                speedHistory = speedHistory.slice(-50);
            }
...
function saveFailedTest() {
    ...
    if (speedHistory.length > 100) {
        speedHistory = speedHistory.slice(-100);
    }
...
function historyload() {
    ...
    reversed.forEach((item) => {
        // Берем данные из сохраненного объекта
...
// 1. Функция для обновления индекса надёжности
function updateConnectIndex() {
    ...
    // Индекс надёжности (0–10)
    const successRate = Math.round((successCount / totalTests) * 100);
    let index = Math.min(Math.round(successRate / 10), 10);
    connectIndexEl.textContent = index;

    // Цвет фона для левой колонки (индекс 0–10)
    let bgColor = 'transparent';
    if (index >= 5) {
        bgColor = 'rgba(46, 213, 115, 0.15)'; // Зелёный
    } else if (index === 4) {
        bgColor = 'rgba(46, 213, 115, 0.10)'; // Светло-зелёный
    } else if (index === 3) {
        bgColor = 'rgba(255, 165, 2, 0.15)';   // Жёлтый
    } else if (index === 2) {
        bgColor = 'rgba(255, 165, 2, 0.10)';   // Светло-жёлтый
    } else if (index === 1) {
        bgColor = 'rgba(255, 71, 87, 0.15)';   // Красный
    } else if (index === 0) {
        bgColor = 'rgba(255, 71, 87, 0.10)';   // Светло-красный
    }
    leftCol.style.background = bgColor;
}

// 2. Функция для обновления количества тестов
function updateTestsCounter() {
    ...
    // Цвет фона для правой колонки (по количеству тестов: 1-5, 5+ = зелёный)
    let bgColor = 'transparent';
    if (totalTests === 0) {
        bgColor = 'transparent';
...
// 3. Функция-обёртка (вызывает обе функции)
