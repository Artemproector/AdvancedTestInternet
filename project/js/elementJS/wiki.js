...
<div class="help-section">
    ...
    <ul>
        <li><strong>DNS</strong> — система, которая переводит адреса сайтов (например, <code>google.com</code>) в IP-адреса. Если DNS не работает — сайты не открываются, даже если интернет есть.</li>
        <li><strong>HTTP</strong> — протокол для передачи данных по открытому каналу. Используется для обычных сайтов. Если HTTP заблокирован — часть сайтов может не открываться.</li>
        <li><strong>HTTPS</strong> — защищённая версия HTTP. Используется большинством современных сайтов. Если HTTPS не работает — большинство сайтов будут недоступны.</li>
    </ul>
    <div class="tip">
        Если DNS не работает, но HTTP и HTTPS работают — возможно, ваш провайдер блокирует DNS-запросы. Попробуйте сменить DNS на публичный (например, Cloudflare 1.1.1.1 или Google 8.8.8.8).
    </div>
</div>
...
<div class="help-section">
    ...
    <div class="mode-item">
        <strong>OK</strong>
        <p>Сайт доступен. Запрос прошёл успешно, сервер ответил.</p>
    </div>

    <div class="mode-item">
        <strong>timeout</strong>
        <p>Сайт не ответил за отведённое время. Это может означать, что сайт заблокирован, сервер перегружен или соединение слишком медленное. Попробуйте увеличить таймаут в настройках.</p>
    </div>

    <div class="mode-item">
        <strong>signal is aborted without reason</strong>
        <p>Сайт начал отвечать, но соединение было разорвано. Это может быть признаком работы ТСПУ (оборудование, которое блокирует трафик).</p>
    </div>

    <div class="mode-item">
        <strong>Failed to fetch</strong>
        <p>Браузер не смог установить соединение с сервером. Сайт может быть заблокирован или недоступен.</p>
    </div>
        ...
        <button onclick="checkNetworkAPI()" style="padding: 10px 20px; background: #2a3555; border: 1px solid #4facfe; border-radius: 8px; color: #fff; cursor: pointer; font-family: Pliant, sans-serif; font-size: 14px; transition: all 0.3s;">
            ...
            <div class="help-section">
                ...
                <ul>
                    ...
                    <li><strong>Обрывать</strong> соединения</li>
